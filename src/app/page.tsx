...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-amber-50 font-sans text-slate-800">
      {/* 상단 헤더 */}
      <header className="bg-orange-500 text-white py-8 px-4 shadow-md sticky top-0 z-20">
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="text-center md:text-left">
            <h1 className="text-3xl font-bold mb-1">🏠 성남시 생활 정보</h1>
            <p className="text-orange-100 italic text-sm">우리 동네 축제, 행사부터 꿀맛 같은 혜택까지!</p>
          </div>
          <nav className="flex gap-4">
            <Link href="/" className="px-4 py-2 bg-white/20 rounded-lg font-bold hover:bg-white/30 transition-colors">홈</Link>
            <Link href="/blog" className="px-4 py-2 bg-orange-600 rounded-lg font-bold hover:bg-orange-700 transition-colors">블로그</Link>
          </nav>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 py-10 space-y-16">
        ...
        <section>
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-2xl font-bold text-orange-700">이번 달 행사/축제</h2>
          </div>
        ...
        <section>
          <div className="flex items-center gap-2 mb-6">
            <span className="text-2xl">💰</span>
            <h2 className="text-2xl font-bold text-amber-700">지원금/혜택 정보</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* 최신 블로그 섹션 */}
        <section className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm transition-all hover:border-orange-200">
          <div className="flex justify-between items-end mb-8">
            <div className="flex items-center gap-2">
              <span className="text-2xl">✍️</span>
              <h2 className="text-2xl font-bold text-slate-800">최신 생활 꿀팁</h2>
            </div>
            <Link href="/blog" className="text-orange-500 font-bold hover:underline transition-all">전체보기 →</Link>
          </div>
          <div className="space-y-6">
            {recentBlogs.map((post) => (
              <Link key={post.id} href={`/blog/${post.id}`} className="group block border-b border-slate-100 last:border-0 pb-6 last:pb-0">
                ...
                <h3 className="text-xl font-bold group-hover:text-orange-600 transition-colors mb-2">{post.title}</h3>
...
function Card({ item, color }: { item: InfoItem; color: "orange" | "amber" }) {
  const bgColor = color === "orange" ? "bg-white" : "bg-white";
  const borderColor = color === "orange" ? "border-orange-100" : "border-amber-100";
  const tagColor = color === "orange" ? "bg-orange-100 text-orange-700" : "bg-amber-100 text-amber-700";
  const btnColor = color === "orange" ? "bg-orange-500 hover:bg-orange-600" : "bg-amber-500 hover:bg-amber-600";
  ...
  return (
    <div className={`p-6 rounded-2xl border-2 ${borderColor} ${bgColor} shadow-sm hover:shadow-md transition-shadow flex flex-col group`}>
      ...
      <Link href={`/info/${item.id}`}>
        <h3 className="text-xl font-bold mb-2 text-slate-800 group-hover:text-orange-600 transition-colors cursor-pointer">{item.name}</h3>
      </Link>
